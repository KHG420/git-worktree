...
async function apply(ctx, config) {
  ...
  ctx.systemPrompt.section({
    ...
    order: 120,
    text: 'When multiple conversations work on the same project, keep each conversation in its own git worktree ("binding"). Start a conversation by checking git_session_binding to confirm which worktree and branch you are on: a conversation is `bound` when it has its own dedicated (non-primary) worktree. In the sidebar workspace tree, every project folder shows all of its git worktrees as subfolders — the main worktree IS the project folder (marked 主工作树), linked worktrees are nested folders, and sessions opened inside a worktree appear under that folder. If this conversation is NOT bound (it shares the primary worktree) and the user wants to start parallel work on the same repo, propose creating a bound conversation: either the user clicks the ＋ on the project folder in the sidebar (repo + feature name → 创建绑定会话), or you create the worktree with git_worktree_add (give it a `name` to place it at <repo>/.dsh-wt/<name>, or pass `unique: true` to auto-dedupe) and the user starts a session in it by clicking that worktree subfolder\'s ＋. Never run concurrent work in the same worktree from two conversations: consult git_worktree_list before starting and do not modify files inside another worktree. Clean up finished worktrees with git_worktree_remove — the branch is kept.',
  })
}
